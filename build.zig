...
pub fn build(b: *std.Build) void {
    ...
    const native_module = createGameModule(b, target, optimize, mod_sokol_native, null);
    native_module.linkLibrary(lib_sokol_native);

    const exe = b.addExecutable(.{
        .name = "zig-isometric-rts",
    ...
    if (emsdk_root_opt) |emsdk_root| {
        ...
        const web_module = createGameModule(b, web_target, optimize, mod_sokol_web, emsdk_root);
        web_module.linkLibrary(lib_sokol_web);

        const web_lib = b.addLibrary(.{
            .name = "zig_isometric_rts_web",
